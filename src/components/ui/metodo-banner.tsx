    ...
    return (
        <div className={`py-8 bg-gray-50 overflow-hidden ${className}`}>
            <div className="relative">
                <div className={`metodo-scroll-track ${reverse ? 'metodo-reverse' : ''}`}>
                    <div className="metodo-scroll-content">
                        {[...methodSteps, ...methodSteps, ...methodSteps].map((step, index) => (
                            <div
                                key={`${step.name}-${index}`}
                                className="metodo-scroll-item"
                            >
                                <div className="flex items-center gap-4 bg-white/80 backdrop-blur-sm rounded-2xl p-4 border border-white/50 shadow-lg min-w-[280px]">
                                    <div className="relative">
            ...
            <div className="relative mt-4">
                ...
                <div className={`metodo-progress-line ${reverse ? 'metodo-progress-reverse' : ''}`} />
            </div>
        </div>
    )
}
